...
/*    private DataSource dataSource;
 ...
 /*       if (user.isNew()) {
    ...
    public boolean deleteUserById(int userId) {
        return jdbcTemplate.update(SQL.DELETE_USER_BY_ID.getQuery(), userId) != 0;
    }

    @Override
    public User getUserById(int userId) {
        List<User> users = jdbcTemplate.query(SQL.GET_USER_BY_ID.getQuery(), ROW_MAPPER, userId);
    ...
    public User validateUser(String email, String inputPassword) {
        ...
        return user.getPassword().equals(inputPassword) ? user : null;
    }
}
